...
for i=1:Nribs+1    
w(1,i)=i-1;
CY(1,i)=(dy*w(1,i))/DesignVariables.Span; %Non-dimensionilized Y
end
...
if CY(1,i)<=0.370000
Scale2(1,i)=(-17.186*CY(1,i)+13.616)*DesignVariables.Scale;
else
Scale2(1,i)=(-7.1904*CY(1,i)+9.9178)*DesignVariables.Scale;
end
end
...
[Coords] = uCRM_MDO;
Coords.Y=Coords.Y*DesignVariables.Scale;
Coords.Z=Coords.Z*DesignVariables.Scale;
Coords.X=Coords.X*DesignVariables.Scale;

for i=1:Nribs+1
w(1,i)=i-1;
%     X(1,i)= -2.3605*CY(1,i).^4+2.4214*CY(1,i).^3+3.0948*CY(1,i).^2+16.612*CY(1,i)+0.0277; % x coord. fit

Y(1,i)= DesignVariables.Span*CY(1,i)+3E-14 ; % y coord. fit
...
%% Create mesh seed in spars' tip edges

Seed{1}=CurveCount(6)+5:NoCurves:CurveCount(6)+NoCurves+5+(Nribs-1)*NoCurves;
Seed{2}=CurveCount(6)+6:NoCurves:CurveCount(6)+NoCurves+6+(Nribs-1)*NoCurves;
fprintf(fidw,'\nmesh_seed_create( "Curve %1.0f", 1, 10, 0., 0., 0. )\n\n',Seed{:});
...
if i==1
    yy(i)=0;
elseif i==Nribs
    yy(i)=DesignVariables.Span;
end
end
ThicknessesMainUpperSkin(1,:)=interp1(DesignVariables.Geometry.Skin.MainUpper.ControlPoints,DesignVariables.Geometry.Skin.MainUpper.Thickness,yy,'linear','extrap');
ThicknessesMainLowerSkin(1,:)=interp1(DesignVariables.Geometry.Skin.MainLower.ControlPoints,DesignVariables.Geometry.Skin.MainLower.Thickness,yy,'linear','extrap');

ThicknessesFrontUpperSkin(1,:)=interp1(DesignVariables.Geometry.Skin.FrontUpper.ControlPoints,DesignVariables.Geometry.Skin.FrontUpper.Thickness,yy,'linear','extrap');
ThicknessesFrontLowerSkin(1,:)=interp1(DesignVariables.Geometry.Skin.FrontLower.ControlPoints,DesignVariables.Geometry.Skin.FrontLower.Thickness,yy,'linear','extrap');
ThicknessesRearUpperSkin(1,:)=interp1(DesignVariables.Geometry.Skin.RearUpper.ControlPoints,DesignVariables.Geometry.Skin.RearUpper.Thickness,yy,'linear','extrap');
ThicknessesRearLowerSkin(1,:)=interp1(DesignVariables.Geometry.Skin.RearLower.ControlPoints,DesignVariables.Geometry.Skin.RearLower.Thickness,yy,'linear','extrap');

ThicknessesFSpar(1,:)=interp1(DesignVariables.Geometry.Spars.FrontSpar.ControlPoints,DesignVariables.Geometry.Spars.FrontSpar.Thickness,yy,'linear','extrap');
ThicknessesRSpar(1,:)=interp1(DesignVariables.Geometry.Spars.RearSpar.ControlPoints,DesignVariables.Geometry.Spars.RearSpar.Thickness,yy,'linear','extrap');
yy = Y;
ThicknessesMainRib(1,:)=interp1(DesignVariables.Geometry.Ribs.MainRib.ControlPoints,DesignVariables.Geometry.Ribs.MainRib.Thickness,yy,'linear','extrap');
ThicknessesFrontRib(1,:)=interp1(DesignVariables.Geometry.Ribs.FrontRib.ControlPoints,DesignVariables.Geometry.Ribs.FrontRib.Thickness,yy,'linear','extrap');
ThicknessesRearRib(1,:)=interp1(DesignVariables.Geometry.Ribs.RearRib.ControlPoints,DesignVariables.Geometry.Ribs.RearRib.Thickness,yy,'linear','extrap');
...
%% Fuel Mass
 [Fuel_Mass_kg] = Fuel_Mass(q2,q3,q4,q5,CY,Rotate2,Scale2,X,Nribs,DesignVariables.Span);
...
fprintf(fidw, '%s\n',S{:});
fprintf(fidw,'%s\n','uil_imaging_coordframes.unpost_all(  )');
fprintf(fidw,'sys_poll_option( 2 )\npoint_label( FALSE )\ncurve_label( FALSE )\nsys_poll_option( 0 )\n');
fprintf(fidw,'uil_toolbar.shaded_smooth(  )');
fprintf(fidw,'%s\n','uil_file_close.go(  )');
...
fclose(fidw);
 ...
 [~,~] = Ribs_Mid_Points(q2,q3,q4,q5,CY,Rotate2,Scale2,X,Y,Z,Nribs,Setup.Settings.No_Spline_Points_Per_Rib);
