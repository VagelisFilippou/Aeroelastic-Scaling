...
selig_url = 'http://airfoiltools.com/airfoil/seligdatfile?airfoil=n63415-il'
# Define a path to save coordinates of the airfoil
selig_path = 'Resources/naca63415-selig.dat'
...
Torsion[0, :] = np.interp(Origin[2, :], [0, SemiSpan], [Root_Torsion, Tip_Torsion])
Scaling[0, :] = np.interp(Origin[2, :], [0, Yehudi, SemiSpan], [Chord_Root, Chord_Yehudi, Chord_Tip])
...
with open('Commands.tcl', 'w') as f:
    ...
    for i in range(0, N_ribs):
        for j in range(0, len(airfoil_x)):
            x_init = airfoil_x[j] * math.cos(Torsion[0, i]) - airfoil_z[j] * math.sin(Torsion[0, i])
            X[i, j] = x_init * Scaling[0, i] + Origin[0, i]

            Y[i, j] = Origin[2, i]

            z_init = airfoil_z[j] * math.cos(Torsion[0, i]) + airfoil_x[j] * math.sin(Torsion[0, i])
    ...
    for i in range(0, 1):
        my_list = [curvecounter[1] + i + 1, curvecounter[2] + i + 1, curvecounter[0] + 2 * (i + 1) + 4 * i,
                   curvecounter[0] + 2 * (i + 1 + 1)]
        my_str = ' '.join(map(str, my_list))
        cmd = "*createmark lines 1 " + my_str
        f.write("*surfacemode 4\n")
        f.write(cmd)
        f.write("\n*surfacesplineonlinesloop 1 0 0\n")  # Check the 64, maybe it is what produces the problem
...
TCLScript = "C:/Users/Vagelis/Documents/UC3M_Internship/Python/UC3M_Parametric_Design/Commands.tcl"
